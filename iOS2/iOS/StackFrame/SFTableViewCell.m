...
-(void) setMessageSFMessage :(SFMessage *) theMessage{
    ...
    if([[_myPost messageStorage] isKindOfClass:[SFChatMessage class]]){
        
        self.chatMessage.text = [[_myPost messageStorage] chatMessage];
    }
...
-(void) addAllMySubViews{
    ...
    if(self.displayPicture != nil)
        [self addSubview:self.displayPicture];
...
-(void) setUpIphoneSix{
    ...
    self.chatMessage.frame = CGRectMake(148/2.0f, 20/2.0f, 450/2.0f, 87/2.0f);
    self.chatMessage.font = theFont;
...
-(void) setUpIphoneSixPlus{
    ...
    self.chatMessage.frame = CGRectMake(82,11,248.4,48.77);
    self.chatMessage.font = theFont;
